            ...
            <Reveal key={group.name} className="skill-group" delay={groupIndex * 0.07}>
              ...
              <div className="skill-list">
                {group.skills.map((skill, skillIndex) => (
                  <motion.div className="skill-item" key={skill.name} initial={{ opacity: 0, scale: 0.88 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true, amount: 0.3 }} transition={{ duration: 0.45, delay: skillIndex * 0.05, ease: [0.22, 1, 0.36, 1] }} whileHover={{ scale: 1.02, x: 4, transition: { type: 'spring', stiffness: 280, damping: 18 } }}>
